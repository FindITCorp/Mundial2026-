...
> Última actualización: **2026-07-02 (R32 10/16 jugados: Bélgica 3-2 remontada en prórroga ✅avance y USA 2-0 ✅ — los goles reales de USA los hicieron los 2 TOP del sim individual, Balogun 1.51 y Tillman; FDH arreglado para R32 → 82 partidos completos; REVISIÓN DEL MODELO con los 10 R32 registrados: sesgo de goles en knockout confirmado (3.75 pred vs 2.60 real) pero corregir λ NO paga (barrido: empeora ganador/Brier, exacto igual) → knob intacto; REGLA NUEVA MEDIDA: MARCADOR→argmax del modelo capado por scoreline_ground (30% exacto vs 11% del experto), GANADOR/AVANCE→experto+ensemble (67% vs 56%); los 6 restantes re-sellados así; Sofascore BLOQUEADO de nuevo → se avanzó solo con FIFA)**
...
- ✅ **CORREGIDO 2-2→2-1 (01-jul, el dueño detectó la inconsistencia):** el 2-2 sellado con XI real NO aparecía ni en el top-6 de marcadores del propio Monte Carlo (top: 1-1 7%, 1-0 7%, 0-1 6%, 2-1 6%, 0-0 6%, 2-0 6%) y además contradecía el campo "ganador"=Bélgica con un marcador de EMPATE — venía de `scoreline_ground` (heurístico de promedios) sin validarlo contra la simulación completa. **Corregido a 2-1 Bélgica** (sí está en el top-6, consistente con Bélgica favorita 45% en 90'). Auditoría del mismo bug en los otros 7 sellos de hoy: TODOS estaban ya dentro de su propio top-6 (Spain-Austria 2-0, Portugal-Croatia 2-1, Switzerland-Algeria 2-1, Australia-Egypt 1-1, Argentina-CaboVerde 2-0, Colombia-Ghana 2-0, USA-Bosnia 2-1) — Bélgica-Senegal era el único caso. **Lección: siempre validar el marcador sellado contra el propio top-6 de `simulate_match`, no solo contra el heurístico de `scoreline_ground`.**
- ✅ **RE-SELLADO CON XI REAL (link del dueño, match-centre FIFA 289287/400021525):** **Bélgica 2-1 Senegal, avance Bélgica ~57%** (id interno 400021526). XI real: **Bélgica 4-3-3** (Courtois; Théate-Mechele-De Cuyper-Castagne; De Bruyne-Tielemans-Vanaken; Trossard-Doku-De Ketelaere) — la formación MÁS EXITOSA del torneo (12V-2E-5D, 2.00 pts/p). **Senegal sorprende con 3-4-3** (línea de 3: Jakobs-Niakhaté-Diatta; Diaw; Gueye-Ciss-Diarra-P.Gueye; Mané-Ndiaye-Sarr) — formación RARA y FRÁGIL en el torneo (n=2, 0.50 pts/p, apenas 0.50 GF/p) pero muestra chica, no generalizar entre equipos. Bajé de la lectura preliminar (72% aún ayer) porque: Elo prácticamente EMPATADO (1733 vs 1735), `opponent_adjust` sigue favoreciendo el proceso de Senegal ajustado por rival (+2.7 vs +2.0, enfrentó a Francia), y Bélgica fue BLANQUEADA por Egipto (su único rival fuerte) — mismo patrón de alerta que España-Austria. Lo que sostiene a Bélgica: XI de gala completo, formación estructuralmente superior, y ventaja clara en el desempate de penales (64% vs 44%, pateadores 81pct vs 22pct). Partido ABIERTO (over2.5 65%, ambos marcan 58%) — **pendiente de evaluar cuando termine (kickoff 20:00Z).**
...
**🎯 NUEVA CAPACIDAD 01-jul — SIMULACIÓN JUGADOR vs DEFENSA (`scripts/player_matchup_sim.py`), pedida explícitamente por el dueño ("simular cuántas oportunidades de gol tendría Kane, cuáles se convertirían, si la defensa lo permitiría"):**
- **Bloqueador resuelto:** los remates de `fifa_match_events` no tenían nombre de jugador (0/1962) — solo `player_fifa_id`. `scripts/fetch_fifa_player_names.py` (nuevo) resuelve vía `api.fifa.com/api/v3/players/{id}` → tabla `fifa_player_names` (725 jugadores cacheados).
- **Modelo:** remates/90 y conversión personales del jugador (Mundial completo) → ajustados por cuánto tiro permite el rival (vs promedio torneo) y por `goals_prevented` del rival (arquero/defensa débil sube conversión) → Monte Carlo remates×conversión → distribución de goles.
- **VALIDADO con datos reales (Harry Kane vs DR Congo, ya jugado, sin fuga vía `--exclude-match`):** modelo daba esperado 1.44 goles / P(≥1)=76.5% / P(2)=24.4%. Real: Kane marcó 2 de 5 remates — dentro de lo que predijo el modelo, no un evento extremo. De paso corrigió un error mío: el gol tempranero de Congo (7') NO fue de Wissa (lo asumí sin verificar) sino de **Brian Cipenga**; ambos goles de England fueron de **Kane** (75',86'), corregido en `fifa_match_goals`.
- **Probado hacia adelante:** Cristiano Ronaldo vs Croacia (R32 pendiente) → esperado 0.99 goles, P(≥1)=63%.
- **Limitación honesta (no se infla el dato):** NO hay regates por jugador en la base (solo agregado de equipo, etiquetado como tal); con muestra chica (ej. Mané, 5 remates) la conversión personal es ruidosa, hay que leerlo con cautela. Perfil defensivo del rival restringido a partidos de Mundial (se encontró que mezclaba amistosos, corregido).
- Uso: `python scripts/player_matchup_sim.py "Jugador" "Equipo rival"` (agregar `--exclude-match {fifa_match_id}` para backtests sin fuga).

**🆕 EXTENSIÓN A TODO EL XI (`scripts/team_lineup_sim.py`) — el dueño pidió "no un jugador, todos los delanteros, todo el mediocampo, todo el juego":** corre la simulación para cada jugador de medio/ataque del XI real de AMBOS equipos y SUMA, cruzando contra el λ de equipo de `simulate_match` (chequeo de consistencia nunca hecho antes). **Encontró un bug real al primer uso:** sumar el XI de Senegal daba 4.27 goles esperados vs 1.67 del modelo de equipo (Ndiaye/Diarra con 1G/2 tiros = 50% conversión, ruido de muestra chica proyectado como tasa real). **Fix: shrinkage bayesiano** (conversión regularizada hacia el promedio del torneo ~12% con peso k=8) — tras el fix, Bélgica cuadra (2.11 vs 2.00) pero **Senegal sigue algo alto (2.68 vs 1.67, pendiente sin resolver del todo, declarado abiertamente**). Resultado de Bélgica-Senegal con XI real (medio+ataque, orden por gol esperado):
  - Bélgica: Trossard 0.70 · De Bruyne 0.67 · Vanaken 0.30 · Tielemans 0.18 · Doku 0.17 · De Ketelaere 0.08 (suma 2.11)
...
**🎯 CORRECCIÓN DE ENFOQUE (01-jul, pedido del dueño: "no podemos evaluar en la marcha, debemos estimar basados en la simulación, sino no tiene sentido predecir"):** en vez de esperar pasivamente a que USA-Bosnia arranque, se generalizó `team_lineup_sim.py`/`full_match_sim.py` con `resolve_xi_source()` — si NO hay XI real del cruce (aún sin alineaciones), cada equipo usa su ÚLTIMO XI propio como proxy (mismo patrón que `analyze_match.py`/`xi_quality.py`), y se corre la simulación completa DE TODAS FORMAS para producir una estimación ahora, no después.
...
**✅ USA-Bosnia RE-SELLADO CON XI REAL (link del dueño, kickoff 00:00Z):** ambos equipos sorprenden con línea de 5 — **USA 5-3-2** (Freese; Dest-Richards-Robinson-Ream-Freeman; Adams-McKennie-Tillman; Pulisic-Balogun) y **Bosnia 5-2-3** (Vasilj; Muharemović-Kolašinac-Dedić-Katić-Radeljić; Gigović-Šunjić; Demirović-Džeko-Alajbegović). **USA 1-0, avance 73%** (antes con proxy: 2-1, avance 66%). Corrido con `full_verdict.py`: modelo de producción 70%/11%/11% vs Monte Carlo+XI real 53%/18%/28% (discrepancia 17pp, reconciliada, no se adoptó el modelo ciego). Balogun concentra casi todo el peligro de USA (goles esperados individuales 1.51 de 2.33 del equipo — killer real, ojo si rota). Bosnia con FDH: USA rompe líneas de ataque por encima del torneo (progresión real).
...
- **Verificado que no rompe nada:** partidos sin XI aún (Spain-Austria) siguen usando el fallback correctamente; `validate_predictions.py --all` sigue en OK tras el cambio.
- **Conclusión: el sello vigente (2-1 Bélgica, 42/23/35, avance ~57%) se sostiene** — la formación real ya estaba factorizada manualmente en el sello de hoy, esto solo lo automatiza para que no dependa de que yo lo revise a mano cada vez. Ningún ajuste numérico adicional justificado con la info de hoy. **Pendiente: evaluar cuando el partido termine (~22:00Z).**

**🛡️ VALIDADOR AUTOMÁTICO NUEVO (`scripts/validate_predictions.py`) — nace de la desconfianza justificada del dueño (01-jul):** tras el fallo de Bélgica-Senegal (2-2 no cuadraba), el dueño señaló que la revisión era manual y ad-hoc ("siempre hay algo que se te olvida... no se automejora"). Construí un gate que corre en TODA la tabla `match_predictions` (no solo lo que se pregunta): probabilidades suman 1, marcador↔ganador coherente, marcador↔goles coherente, marcador dentro del top-8 real de `simulate_match`. **Al correrlo sobre TODO encontró el MISMO bug en Australia-Egipto (id 400021531: sellado '1-1' pero ganador='Egypt' en vez de 'Draw') sin que nadie lo señalara — corregido con `--fix`.** Distingue el pipeline automático de GitHub Action (verificado en el código de `predict_upcoming.py`: ahí winner=argmax de 1X2 agregado y scoreline=argmax del grid Dixon-Coles, DIVERGEN por diseño, no es bug) de mis sellos expertos (ahí exijo coherencia total). **Este validador es ahora paso OBLIGATORIO tras sellar, no opcional — es la respuesta concreta a "no se automejora": ya no dependo de que alguien note el error.**
